...
from utils import (linear_scale_forward, hnn_erp_param_function, UniformPrior, linear_scale_array,
...
theta_cond_list = [np.array([0.5, 0.5, 0.75, 0.75]), np.array([0.5, 0.5, 0.5, 0.5]), np.array([0.55, 0.5, 0.8, 0.75]),
...
for plot_idx, theta_cond in enumerate(theta_cond_list):
    ...
    for input_type, posterior_dict in posterior_state_dicts.items():
        ...
        all_labels = ['Distal inh (log g)', 'Proximal inh (log g)', 'Distal exc (log g)', 'Proximal exc (log g)']
